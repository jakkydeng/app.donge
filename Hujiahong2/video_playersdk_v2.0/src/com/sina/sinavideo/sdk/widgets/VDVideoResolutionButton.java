 ...
 * 
 */
public class VDVideoResolutionButton extends TextView implements VDBaseWidget, OnResolutionListener,
        OnShowHideControllerListener, OnShowHideBottomControllerListener {
    ...
    public VDVideoResolutionButton(Context context, AttributeSet attrs) {
        ...
        mAlignCenter = a.getBoolean(R.styleable.ResolutionBackGround_alignCenter, true);
        a.recycle();
        registerListeners();
        init(context);
    }

    public VDVideoResolutionButton(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        ...
        setOnFocusChangeListener(new OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if (!hasFocus) {
                    VDVideoViewController.getInstance(mContext).notifyResolutionContainerVisible(false);
                }
            }
        });
    ...
    private void setListButtonVisible(boolean isVisible) {
        VDVideoViewController controller = VDVideoViewController.getInstance(VDVideoResolutionButton.this.getContext());
        if (mAlignCenter) {
            updateResolutionPos();
        }
        if (controller != null) {
    ...
    private void registerListeners() {
        ...
        this.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                VDVideoViewController controller = VDVideoViewController.getInstance(VDVideoResolutionButton.this
                        .getContext());
    ...
    public void onResolutionParsed(VDResolutionData list) {
        ...
        if (list != null && list.getResolutionSize() > 0) {
            String tag = VDResolutionManager.getInstance(mContext).getCurrResolutionTag();
            if (tag != null) {
                if (VDResolutionData.TYPE_DEFINITION_FHD.equals(tag)) {
                    setText(R.string.super_definition);
                } else if (VDResolutionData.TYPE_DEFINITION_HD.equals(tag)) {
