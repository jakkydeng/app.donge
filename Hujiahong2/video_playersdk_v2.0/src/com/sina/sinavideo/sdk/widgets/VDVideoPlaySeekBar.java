 ...
 * 
 */
public class VDVideoPlaySeekBar extends SeekBar implements VDBaseWidget, VDVideoViewListeners.OnProgressUpdateListener,
        SeekBar.OnSeekBarChangeListener, VDVideoViewListeners.OnBufferingUpdateListener {
    ...
    public void onProgressUpdate(long current, long duration) {
        // LogS.e(TAG, "DLNA onProgressUpdate   current:" + current +
        // " , duration = " + duration + " , max = " + getMax());
    ...
    public void onStartTrackingTouch(SeekBar seekBar) {
        ...
        if (!DLNAController.mIsDLNA) {
            ...
            if (playInfo != null) {
                ...
                // getProgress();
                VDSDKLogData.getInstance().mSeekFrom = getProgress();
            }
        }
        controller.notifyHideControllerBar(VDVideoViewController.DEFAULT_DELAY);
    }
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        // VDVideoViewController.getInstance().mSeekTo = getProgress();
        VDSDKLogData.getInstance().mSeekTo = getProgress();
        VDVideoViewController controller = VDVideoViewController.getInstance(this.getContext());
        if (controller != null)
            controller.notifyHideControllerBar(VDVideoViewController.DEFAULT_DELAY);
        if (DLNAController.mIsDLNA) {
        ...
        if (controller != null) {
            controller.mLogHighPingTag = true;
            VDSDKLogManager.getInstance().onSeek(controller.mVDVideoListInfo.mIndex);
        }
    }
