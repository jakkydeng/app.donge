...
public class VDPlayerSoundManager {
     ...
     *            是否有通知
     */
    public static void dragSoundSeekTo(Context context, int currVolume, boolean notify) {
        if (DLNAController.mIsDLNA) {
            ...
            if (getAudioManager(context) != null) {
                getAudioManager(context).setStreamVolume(AudioManager.STREAM_MUSIC, currVolume, 0);
            }
        }
        VDVideoViewController controller = VDVideoViewController.getInstance(context);
        if (null != controller) {
            controller.mVDPlayerInfo.mCurVolume = currVolume;
            VDSDKLogManager.getInstance().onSoundChange(controller.mVDVideoListInfo.mIndex);
            if (notify) {
    ...
    public static boolean isMuted(Context context) {
        ...
        if (audioManager != null) {
            currSoundNum = audioManager.getStreamVolume(AudioManager.STREAM_MUSIC);
        }
     ...
     * @param isMuted
     */
    public static void setMute(Context context, boolean isMuted, boolean needNotify) {
        AudioManager audioManager = (AudioManager) context.getSystemService(Service.AUDIO_SERVICE);
        if (audioManager == null) {
            return;
        }
        if (isMuted) {
            mCurrSoundNum = audioManager.getStreamVolume(AudioManager.STREAM_MUSIC);
