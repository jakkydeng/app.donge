...
public class VDVideoViewListeners {
    ...
    public void removeOnResolutionContainerListener(OnResolutionContainerListener l) {
    ...
    public void addOnShowHideTopContainerListener(OnShowHideTopContainerListener l) {
        mOnShowHideTopContainerListener.add(l);
    }

    public void removeOnShowHideTopContainerListener(OnShowHideTopContainerListener l) {
    ...
    void notifyScreenSingleTouch(final MotionEvent ev, final eSingleTouchListener flag) {
        mMainHandler.post(new Runnable() {
    ...
    void notifyScreenDoubleTouch(final MotionEvent ev, final eDoubleTouchListener flag) {
        mMainHandler.post(new Runnable() {
    ...
    private float getCurrTimeFromEvent(final PointF point1, final PointF point2) {
        ...
        DLNAController dlnaController = DLNAController.getInstance(controller.getContext());
        ...
        float rate = DLNAController.mIsDLNA ? dlnaController.mProgressRate : controller.mProgressRate;
        float ret = ((float) current / duration) + ((float) distance / (float) width) * rate;
        VDLog.e(TAG, "current : " + current + ",duration : " + duration + " , distance : " + distance + " , ret = "
                + ret);
        if (ret < 0) {
            ret = 0;
        } else if (ret > 1) {
    ...
    private float getCurrSoundFromEvent(final PointF point1, final PointF point2, float distansY) {
        ...
        if (DLNAController.mIsDLNA) {
            ...
            maxVolume = VDPlayerSoundManager.getMaxSoundVolume(controller.getContext());
            currVolume = VDPlayerSoundManager.getCurrSoundVolume(controller.getContext());
        }
        ...
        Log.i("getCurrSoundFromEvent", "fromDownY = " + distansY + " , currVolume = " + currVolume + " , degree = "

        + degree + " , tmp_stream_level = " + mTmpStreamLevel + " , maxVolume = " + maxVolume);
        mTmpStreamLevel += (degree * maxVolume);
        // float ret = (currVolume+degree*maxVolume);
        Log.d("getCurrSoundFromEvent", "tmp_stream_level = " + mTmpStreamLevel + " , add = " + (degree * maxVolume));
     ...
     * @return
     */
    private float getCurrLightingFromEvent(final PointF point1, final PointF point2) {
        float ret;
        float y1 = point1.y;
        float y2 = point2.y;
        VDVideoViewController controller = VDVideoViewController.getInstance(mContext);
    ...
    void notifyScreenVerticalScrollTouch(final PointF point1, final PointF point2, final PointF beginPoint,
                ...
                switch (flag) {
                    case eTouchListenerVerticalScrollStart :
                        ...
                        if (isSoundVisible) {
                            ...
                            if (DLNAController.mIsDLNA) {
                                ...
                                if (controller != null)
                                    mTmpStreamLevel = DLNAController.getInstance(controller.getContext()).mVolume;
                            } else {
                                Context context = VDApplication.getInstance().getContext();
                                mTmpStreamLevel = VDPlayerSoundManager.getCurrSoundVolume(context);
                            }
                            VDLog.e("getCurrSoundFromEvent", "tmp_stream_level = " + mTmpStreamLevel);
                        } else {
                    ...
                    case eTouchListenerVerticalScroll :
                        ...
                        if (getIsRight(beginPoint)) {
                            ...
                            VDLog.e(TAG, "notifyScreenVerticalScrollTouch,eTouchListenerVerticalScroll,dragLightingTo");
                            if (controller != null)
                                controller.dragLightingTo(curr, true);
                        }
                        break;
    ...
    void notifyScreenHorizonScrollTouch(final PointF point1, final PointF point2, final PointF beginPoint,
                ...
                switch (flag) {
                    case eTouchListenerHorizonScrollStart :
                        if (!videoInfo.mIsLive) {
                            ...
                            if (DLNAController.mIsDLNA) {
                                ...
                                controller.mVDPlayerInfo.mCurrent = controller.getCurrentPosition();
                                VDPlayerInfo playInfo = controller.getPlayerInfo();
                                if (playInfo != null) {
                                    // VDVideoViewController.getInstance().mSeekFrom
                                    // = playInfo.mCurrent;
                                    VDSDKLogData.getInstance().mSeekFrom = playInfo.mCurrent;
                                }
                    ...
                    case eTouchListenerHorizonScrollEnd :
                        if (!videoInfo.mIsLive) {
                            ...
                            controller.dragProgressTo(getCurrTimeFromEvent(point1, point2), true, true);
                            if (DLNAController.mIsDLNA) {
        ...
        mMainHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (videoInfo != null && !videoInfo.mIsLive) {
                    ...
                    VDLog.e(TAG, "current : " + current + ",duration : " + duration + " , ret = " + ret);
                    if (ret < 0) {
                        ret = 0;
                    } else if (ret > 1) {
                        ret = 1;
