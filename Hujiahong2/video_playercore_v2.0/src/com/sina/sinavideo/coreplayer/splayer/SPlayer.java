...
public class SPlayer {
    ...
    private static final String[] oldLibrarys = {"libffmpeg.so", "libOMX.11.so", "libOMX.14.so", "libOMX.18.so",
            "libOMX.9.so",
            // "libstlport_shared.so",
            "libvao.0.so", "libvplayer.so", "libvscanner.so", "libvvo.0.so", "libvvo.7.so", "libvvo.8.so",
            "libvvo.9.so", "libvvo.j.so"};
    ...
    public static boolean initialize(Context ctx) {
        return isInitialized(ctx) || extractLibs(ctx,ctx.getResources().getIdentifier("libsplayer", "raw", "com.sina.video_playersdkv2"));
    }
        ...
        if (dir.exists() && dir.isDirectory()) {
            ...
            if (libs != null) {
                ...
                try {
                    ...
                    Log.i(TAG, "isNativeLibsInited, APP VERSION: " + appVersion + ", SPLayer Library version: "
                            + libVersion);
    ...
    private static boolean extractLibs(Context ctx, int rawID) {
        ...
        Log.d(TAG, "copyCompressedLib time: " + (System.currentTimeMillis() - begin) / 1000.0);
        boolean inited = initializeLibs(libPath, getLibraryPath(), String.valueOf(SPlayer.getVitamioType()));
        ...
        try {
            ...
            Log.d(TAG, "initializeNativeLibs: " + inited + ", libsType:" + SPlayer.getVitamioType());
            Log.d(TAG, "loadLibs time: " + (System.currentTimeMillis() - begin) / 1000.0);
    ...
    private native static boolean initializeLibs(String libPath, String destDir, String prefix);
}
