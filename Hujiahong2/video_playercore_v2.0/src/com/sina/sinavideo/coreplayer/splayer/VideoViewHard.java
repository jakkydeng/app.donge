...
//        LogS.i("videoView", "onAttachedToWindow " + getWindowToken() + " , mChangeParentView = " + mChangeParentView);
...
//        LogS.i("videoView", "onDetachedFromWindow " + getWindowToken() + " , mChangeParentView = " + mChangeParentView);
        if (mChangeParentView) {
...
//        LogS.i("videoView", "onWindowVisibilityChanged " + getWindowToken() + " , mChangeParentView = "
//                + mChangeParentView);
        if (mChangeParentView) {
    ...
    SurfaceHolder.Callback mSHCallback = new SurfaceHolder.Callback() {
        ...
        public void surfaceCreated(SurfaceHolder holder) {
            log("surfaceCreated() into, mCurrentState " + mCurrentState + " mTargetState " + mTargetState);
            log("surfaceCreated()  holder=[" + holder + "],holder.getSurface()=[" + holder.getSurface() + "]");
        ...
        public void surfaceDestroyed(SurfaceHolder holder) {
            log("surfaceDestroyed() into,mCurrentState=" + mCurrentState + ",mTargetState=" + mTargetState);
    ...
    private void openVideo() {
        ...
        if (false == sPlayerIsInitialized) {
            ...
            if (leftSize >= AndroidUtil.MIN_PLAYSDK_SOTROAGRE_SIZE) {
                sPlayerIsInitialized = SPlayer.initialize(mContext, R.raw.libsplayer);
            } else {
                Toast.makeText(this.getContext(), "您的设备存储空间不足,将无法正常播放视频,请清理后重试", Toast.LENGTH_SHORT).show();
        ...
        if (mUri == null || mSurfaceHolder == null) {
            log("openVideo() donothing for NULL mUri=" + mUri + ",mSurfaceHolder=" + mSurfaceHolder);
            return;
        }
        ...
        try {
            ...
            log("openVideo() mSurfaceHolder=[" + mSurfaceHolder + "],mSurfaceHolder.getSurface()=["
            ...
            mMediaPlayer.setWakeMode(mContext, PowerManager.SCREEN_BRIGHT_WAKE_LOCK);
            ...
            LogS.e(TAG, "catch other exception,Unable to open content: " + mUri, ex);
            ex.printStackTrace();
            mCurrentState = STATE_ERROR;
            mTargetState = STATE_ERROR;
            mErrorListener.onError(mMediaPlayer, MediaPlayer.MEDIA_ERROR_UNKNOWN, 0);
    ...
    public void suspend() {
        log("suspend() into");
        if (isInPlaybackState() && mCurrentState != STATE_PAUSED && mTargetState != STATE_PAUSED) {
...
//    }
        ...
        public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
            ...
            if (mVideoWidth > 0 && mVideoHeight > 0) {
                ...
                if (mOnVideoSizeChangedListener != null) {
                    mOnVideoSizeChangedListener.onVideoSizeChanged(mMediaPlayer, mVideoWidth, mVideoHeight);
                }
            }
        }
    };
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        log("onMeasure() into" + "widthMeasureSpec  " + MeasureSpec.getSize(widthMeasureSpec)
        ...
        if (p_lp != null) {
            ...
            if (p_lp.width == -1) {
                windowWidth = disp.widthPixels;// /windowWidth =
                                               // p.getMeasuredWidth();
//                LogS.d(TAG, "p_lp.width==-1,p.getMeasuredWidth()=" + windowWidth);
            } else {
            ...
            if (p_lp.height == -1) {
                windowHeight = disp.heightPixels;// /windowHeight =
                                                 // p.getMeasuredHeight();
//                LogS.d(TAG, "p_lp.height==-1,p.getMeasuredHeight()=" + windowHeight);
            } else {
                windowHeight = p_lp.height;
            }
        } else {
//            LogS.d(TAG, "p_lp NULL,windowWidth=" + disp.widthPixels + ",windowHeight=" + disp.heightPixels);
        }
        ...
        if (VIDEO_SCALE_ORIGIN == layout && mSurfaceWidth < windowWidth && mSurfaceHeight < windowHeight) {
            ...
            lp.width = (full || windowRatio < videoRatio) ? windowWidth : videoRatio == 0
                    ? windowWidth
                    : (int) (videoRatio * windowHeight);
            lp.height = (full || windowRatio > videoRatio) ? windowHeight : videoRatio == 0
                    ? windowHeight
