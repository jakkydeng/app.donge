...
public class M3u8ContentParser {
    ...
    public M3u8ContentParser(M3u8ParserListener listener, String _vid, Context ctt) {
        mListener = listener;
    ...
    private VDResolutionData parse(InputStream is) {
        ...
        try {
            ...
            String resolutionType = VDResolutionData.TYPE_DEFINITION_SD;
            String line = null;
            int idx = 0;
            ...
            while ((line = br.readLine()) != null) {
                if (line.startsWith("#EXT-X-STREAM-INF")) {
                    ...
                    // resolution.mResolutionType = resolutionType++;
                    ...
                    // http url for resolution list
                    if (m3u8Type == M3u8Content.M3U8_TYPE_RESOLUTION && resolution != null) {
                        resolution.setUrl(line);
                        if (resolutionData == null) {
                            resolutionData = new VDResolutionData();
                        }
                        // list.add(resolution);
                        resolutionData.addResolution(resolutionTag, resolution);
                        resolution = null;
                    }
                    idx++;
                } else {
            ...
            if (m3u8Type == M3u8Content.M3U8_TYPE_RESOLUTION && resolutionData != null) {
                // M3u8ResolutionContent content = new M3u8ResolutionContent();
                // content.setResolutionList(list);
                // return content;
        ...
        private VDResolutionData retryConnect() {
            for (int i = 0; i < RETRY_TIMES; i++) {
                ...
                try {
                    ...
                    request.addHeader("User-Agent", "sinavideo/2.0.1 CFNetwork/672.0.2 Darwin/14.0.0");
                    request.setHeader("Accept-Encoding", "gzip, deflate");
                    request.setHeader("Accept-Language", "zh-cn");
                    request.setHeader("Accept", "*/*");
                    HttpResponse response = sHttpClient.execute(request);
                    int statusCode = response.getStatusLine().getStatusCode();
                    LogS.w(TAG, "M3u8Content retryConnect statusCode = " + statusCode);
